    ...
    <DialogContent className="flex flex-col sm:flex-row p-2">
      <div className="flex flex-auto flex-col py-4 px-0 sm:px-4">
          ...
          <div className="flex-1 mb-6 mx-2">
            ...
            <Autocomplete
              ...
              onChange={(_event: SyntheticEvent<Element, Event>, value: (string | ScrumboardLabel)[]) => {
                const ids = value.filter((item): item is ScrumboardLabel => typeof item !== 'string').map((item) => item.id);
                setValue('labels', ids);
              }}
              renderTags={(value, getTagProps) =>
                value.map((option, index) => {
                  const { key, ...rest } = getTagProps({ index });
                  return <Chip key={key} label={typeof option === 'string' ? option : option?.title} className="m-0.75" {...rest} />;
                })
              }
              renderInput={(params) => (
                <TextField
        ...
        {cardForm.memberIds && cardForm.memberIds.length > 0 && (
          <div className="flex-1 mb-6 mx-2">
            ...
            <Autocomplete
              ...
              onChange={(_event: SyntheticEvent<Element, Event>, value: (string | ScrumboardMember)[]) => {
                const ids = value.filter((item): item is ScrumboardMember => typeof item !== 'string').map((item) => item.id);
          ...
          cardForm.checklists.map((checklist, index) => (
            <CardChecklist
              ...
              onCheckListChange={(item, itemIndex) => {
                setValue('checklists', setIn(cardForm.checklists, `[${itemIndex}]`, item) as ScrumboardChecklist[]);
              }}
              onRemoveCheckList={() => {
        ...
        <div className="mb-6">
          ...
          <div>
            <CardComment onCommentAdd={(comment) => setValue('activities', [comment, ...cardForm.activities] as ScrumboardComment[])} />
          </div>
        </div>
      ...
      <div className="flex order-first sm:order-last items-start sticky top-0">
        <Box className="flex flex-row sm:flex-col items-center rounded-lg w-full overflow-hidden" sx={{ backgroundColor: 'background.default' }}>
          <IconButton className="order-last sm:order-first rounded-none" color="inherit" onClick={() => dispatch(closeCardDialog())} size="large">
          ...
          <div className="flex flex-row items-center sm:items-start sm:flex-col flex-1">
            <Controller
              name="dueDate"
              control={control}
              render={({ field: { onChange, value } }) => <DueMenu onDueChange={onChange} onRemoveDue={() => onChange(null)} dueDate={value} />}
            />
            ...
            <Controller
              ...
              defaultValue={[]}
              render={() => (
                <IconButton size="large" className="rounded-none">
            ...
            <Controller
              ...
              control={control}
              defaultValue={[]}
              render={({ field: { onChange } }) => <CheckListMenu onAddCheckList={(newList) => onChange([...cardForm.checklists, newList])} />}
            />
