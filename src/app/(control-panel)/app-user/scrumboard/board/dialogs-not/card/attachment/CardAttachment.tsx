...
function CardAttachment(props: CardAttachmentProps) {
  ...
  switch (item.type) {
    case 'image': {
      return (
        <div className="flex w-full sm:w-1/2 mb-4 px-4" key={item.id}>
          ...
          <div className="flex flex-auto flex-col justify-center items-start min-w-0 px-4">
            <div className="flex items-center w-full">
              ...
              {card.attachmentCoverId === item.id && (
                <FuseSvgIcon className="text-orange-300 mx-1" size={20}>
                  heroicons-outline:start
                </FuseSvgIcon>
              )}
            </div>
            <Typography className="truncate w-full mb-3" color="text.secondary">
